...
function RatingContents(){
    ...
    const getUserInfo=()=>{
        ...
        axios({
            ...
            reponse=>{
                ...
                setMemberId(reponse.data.kakao_account.email)
    ...
    const enrollComment=async()=>{
        ...
        const rating = document.getElementById('rating-num').innerText
        console.log('등록평점')
        console.log(rating)
        const commentInfo
        ={movie_id:movieId,member_id:memberId,nickname:nickName,comment:comment,rating:rating,write_date:'2023-07-01 12:30:20'}
        const result = await axios.post('/api/post/comment',commentInfo)
        setCommentList([commentInfo,...commentList])
    }

    const getCommentListLatest=async()=>{
        const result = await axios.get('/api/get/comment/latest')
    ...
    const getCommentListOldest=async()=>{
        ...
        const result = await axios.get('/api/get/comment/oldest')
        console.log('getCommentListOldest')
        console.log(result)
        console.log(result.data.comment_res)

        if(isLoggedIn===true){
            const result=await axios.get(`/api/get/comment/history/${memberId}`)
            //댓글 추천 비추천 이력 가지고옴
    ...
    const getMoreComment=async()=>{
        ...
        const result = await axios.get(`/api/get/comment/more/${commentPage}`)
        console.log(result)
